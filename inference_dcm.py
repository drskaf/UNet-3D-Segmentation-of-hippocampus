...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    orig1 = np.flip((orig_vol[20, :, :]/np.max(orig_vol[20, :, :]))*0xff).T.astype(np.uint8)
    orig2 = np.flip((orig_vol[:, 25, :]/np.max(orig_vol[:, 25, :]))*0xff).T.astype(np.uint8)
    
    
    #for s in range(3):
        #_slice = slice_nums[s]
        #orig = np.flip((orig_vol[_slice] / orig_vol[_slice].max())*0xff).T.astype(np.uint8)
        #pred = np.flip((pred_vol[_slice] / pred_vol[_slice].max())*0xff).T.astype(np.uint8)
        
    # This is how you create a PIL image from numpy array
    pil_i1 = Image.fromarray(orig1, mode="L").convert("RGBA").resize(size=(400,400))
    pil_i2 = Image.fromarray(orig2, mode="L").convert("RGBA").resize(size=(400,400))
    ...
    mask = Image.new('RGBA',pil_m1.size,(0,0,0,123))
    pil_im1 = Image.composite(pil_m1,pil_i1,mask).convert('RGB')
    
    pred2 = np.flip((pred_vol[:, 25, :]/np.max(pred_vol[:, 25, :]))*0xff).T.astype(np.uint8)
    pil_m2 = Image.fromarray(pred2, mode="L").convert("RGBA").resize(size=(400,400))
    data = np.array(pil_m2)
    red, green, blue, alpha = data.T
    white_areas = (red == 255) & (blue == 255) & (green == 255)
    data[..., :-1][white_areas.T] = (255, 0, 0)
    pil_m2 = Image.fromarray(data)
    mask = Image.new('RGBA',pil_m2.size,(0,0,0,123))
    pil_im2 = Image.composite(pil_m2,pil_i2,mask).convert('RGB')
    
    
    # Paste the PIL image into our main report image object (pimg)
    pimg.paste(pil_im1, box=(10,500))
    pimg.paste(pil_im2, box=(420, 500))
...
if __name__ == "__main__":
    ...
    os_command("storescu 127.0.0.1 4242 -v -aec HIPPOAI +r +sd /data/TestVolumes/Study1")
